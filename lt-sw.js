...
const SHELL = ['./index.html','./lt-manifest.json','./lt-icon-192.png','./lt-icon-512.png'];
...
self.addEventListener('message', e => {
  ...
  if (e.data && e.data.type === 'NOTIFY') {
    const { title, body, tag, icon, urgent } = e.data;
    self.registration.showNotification(title, {
      ...
      tag: tag || 'lifetrack',
      renotify: true,
      vibrate: urgent ? [400, 100, 400, 100, 800] : [200, 100, 200],
  ...
  e.waitUntil(
    self.clients.matchAll({ type: 'window', includeUncontrolled: true }).then(clients => {
      for (const client of clients) {
        if (client.url.includes('lifetrack') || client.url.includes('index') && 'focus' in client) return client.focus();
      }
